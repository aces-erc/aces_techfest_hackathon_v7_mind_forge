...
export default function UserLocationHospital() {
    const [locations, setLocations] = useState([])
    const [userLocation, setUserLocation] = useState(null)
    const [locationError, setLocationError] = useState(null)
    const { socket, patientDetail } = useSocket();
    const { user, incommingPatient } = useUserStore();


    useEffect(() => {

        const userPos = {
            lat: user.hospitalLocation.latitude,
            lng: user.hospitalLocation.longitude
        };
    ...
    useEffect(() => {
        if (incommingPatient) {
            const data = [
                incommingPatient.location
            ]
            setLocations(data);
        }
    }, [incommingPatient, socket])
